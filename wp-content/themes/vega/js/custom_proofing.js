	...
	jQuery('.image_approve').click(function(){
		...
		if(jQuery('#gallery_proofing_status').val() == 0)
		{
			...
			jQuery.ajax({
		        url: tgAjax.ajaxurl,
		        type:'POST',
		        data: 'action=vega_image_proofing&method=approve&gallery_id='+galleryID+'&image_id='+imgID+'&tg_security='+tgAjax.ajax_nonce, 
	...
	jQuery('.image_unapprove').click(function(){
		...
		if(jQuery('#gallery_proofing_status').val() == 0)
		{
			...
			jQuery.ajax({
		        url: tgAjax.ajaxurl,
		        type:'POST',
		        data: 'action=vega_image_proofing&method=unapprove&gallery_id='+galleryID+'&image_id='+imgID+'&tg_security='+tgAjax.ajax_nonce, 
		        success: function(html)
		        {
